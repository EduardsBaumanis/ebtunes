...
Open [`apps/lofi-player/supabase.js`](../apps/lofi-player/supabase.js) and replace the
...
1. From the repo root, start the compatibility server:
   ```bash
   npm install
   npm start
   ```
...
Commit and push, and Pages will publish the player at `/lofi-player/` from
`apps/lofi-player/`.
