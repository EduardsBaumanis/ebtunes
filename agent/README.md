...
By default the agent writes a `.strudel` file to `agent/output/` and
prints the path on stdout. With `--stdout`, the pattern itself is
written to stdout and nothing is saved to disk.

### Live in lofi-player

Drop a generated file into `lofi/`, `jazz-piano/`, `study/`, or any other
playlist folder, then add the filename to the corresponding entry in
`lofi-player/playlists.js`. The pattern shows up as a track on the next
page reload.
